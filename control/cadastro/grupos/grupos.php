...
class grupos extends xajaxResponse{
	private $_c;
	private $con;
	private $smarty;
	private $p1;
	private $p2;
	private $arquivo='control/cadastro/grupos/grupos';
	...
	public function base(){
		$tpl=$this->smarty->fetch('views/templates/cadastro/grupos/base.tpl');
		$s=$this->smarty->fetch('views/templates/cadastro/grupos/base.js');
	...
	public function listar(){
		...
		$sql=sprintf("SELECT id, ativo, login, senha, nome, email, telefone, celular1 FROM grupos WHERE del<>'s' %s",$valor);
		...
		$tpl=$this->smarty->fetch('views/templates/cadastro/grupos/listar.tpl');
		$s=$this->smarty->fetch('views/templates/cadastro/grupos/listar.js');
	...
	public function editar(){
		try {
			...
			if ($editar){	
				$sql="SELECT id, ativo, login, senha, nome, email, telefone, celular1 FROM grupos WHERE del<>'s' AND id=?";
			...
			$tpl=$this->smarty->fetch('views/templates/cadastro/grupos/form.tpl');
			$s=$this->smarty->fetch('views/templates/cadastro/grupos/form.js');
			$this->assign('divForm', 'innerHTML', $tpl);
			$this->script($s);
		} catch (Exception $e) {
	...
	public function salvar(){
		...
		try {
			...
			$sql=$op." grupos SET ativo=?, login=?, senha=?, nome=?, email=?, telefone=?, celular1=? ".$where;
			$sql=$this->con->Prepare($sql);
	...
	public function excluir(){
		try{
			$sql="UPDATE grupos SET del=? WHERE id=?";
			$sql=$this->con->Prepare($sql);
			$_d['del']='s';
			$_d['id']=$this->p1;
			if (!($rs=$this->con->Execute($sql, $_d))){
	...
	public function ativo(){
		$sql="UPDATE grupos SET ativo=? WHERE id=?";
			$sql=$this->con->Prepare($sql);
			if ($this->p2=='s'){
				$t='Desativado';
				$_d['ativo']='n';
